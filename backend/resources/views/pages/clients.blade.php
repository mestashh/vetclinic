...
@section('content')
    <div class="p-4">
        ...
        <form method="GET" action="{{ route('clients') }}" class="mb-4">
            <div class="flex">
                <input type="text" name="search" value="{{ $search ?? '' }}"
                       placeholder="Поиск по ФИО" class="border rounded p-2 flex-grow mr-2">
            ...
            <table class="min-w-full bg-white shadow rounded-lg">
                ...
                @forelse($clients as $client)
                    <tr class="border-t">
                        ...
                        <td class="px-4 py-2">{{ $client->first_name }} {{ $client->middle_name }} {{ $client->last_name }}</td>
                ...
                @empty
                    <tr>
                        <td colspan="5" class="px-4 py-2 text-center">Клиентов пока нет.</td>
                    </tr>
                @endforelse
                </tbody>
            </table>
